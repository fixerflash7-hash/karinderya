...
local function updateIngredientLabel(ingredKey)
    ...
    -- For Orion, we need to update the paragraph
    -- We'll store the paragraph object and update its content
...
-- HOME TAB
HomeTab:AddLabel("Thank you for choosing to use my script!")
HomeTab:AddLabel("Development: 5 days of development, testing, and improvements.")
HomeTab:AddLabel("Add & Follow Me On: Roblox: KnorkzykiiPH & Tiktok: _jsephmols")
HomeTab:AddLabel("Creator: _jsephmols")
...
HomeTab:AddButton({
    ...
    Callback = function()
        if pcall(function()
            ...
            OrionNotify("Discord Link Copied", "Discord invite link copied to clipboard.", 3)
        else
...
HomeTab:AddParagraph("Features", "Auto Serve Food & Auto Assign Customers | Auto Collect Trash | Auto Serve Drinks (Requires Chiller) | Auto Wash Dishes | Auto Claim Codes | Auto Unlock Menu | Auto Buy Ingredients | Auto Unlock Tables | AND MORE")
HomeTab:AddParagraph("Player Features", "WalkSpeed | Infinite Jump | NoClip | Anti AFK | Teleports")
HomeTab:AddParagraph("Utilities", "Config Save/Load | Delay Settings")
...
PlayerTab:AddButton({
    ...
    Callback = function()
        ...
        OrionNotify("Anti AFK", "Anti AFK is now running!", 4.5)
    end
})
...
for _, ingredKey in ipairs(sortedIngredients) do
    ...
    -- Store paragraph reference for updates
...
local tableDropdown = ShopTab:AddDropdown({
    Name = "Select Tables",
    Default = "",
...
local chairDropdown = ShopTab:AddDropdown({
    Name = "Select Chairs",
    Default = "",
...
local materialDropdown = ShopTab:AddDropdown({
...
local paintDropdown = ShopTab:AddDropdown({
    Name = "Select Paints",
    Default = "",
...
local tileDropdown = ShopTab:AddDropdown({
    Name = "Select Tiles",
    Default = "",
...
local stoveDropdown = ShopTab:AddDropdown({
    Name = "Select Stoves",
    Default = "",
...
for _, group in ipairs(shopSections) do
    ...
    for _, entry in ipairs(sortedItems) do
        ...
        local itemSection = section:AddSection(displayName)
...
task.defer(function()
    task.wait(0.5)
    OrionNotify("Script Loaded", "Thank you for choosing to use my script", 8)
end)
