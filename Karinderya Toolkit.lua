...
homeSection:AddLabel("Thank you for choosing to use my script!")
homeSection:AddLabel("Development: 5 days of development, testing, and improvements.")
homeSection:AddLabel("Add & Follow Me On: Roblox: KnorkzykiiPH & Tiktok: _jsephmols")
homeSection:AddLabel("Creator: _jsephmols")
...
homeSection:AddLabel("Features: Auto Serve Food & Auto Assign Customers | Auto Collect Trash | Auto Serve Drinks (Requires Chiller) | Auto Wash Dishes | Auto Claim Codes | Auto Unlock Menu | Auto Buy Ingredients | Auto Unlock Tables | AND MORE")
homeSection:AddLabel("Player Features: WalkSpeed | Infinite Jump | NoClip | Anti AFK | Teleports")
homeSection:AddLabel("Utilities: Config Save/Load | Delay Settings")
...
local autoServeToggle = mainSection1:AddToggle({
...
local autoOrderToggle = mainSection1:AddToggle({
...
local autoSoftdrinksToggle = mainSection1:AddToggle({
...
local autoRejectToggle = mainSection1:AddToggle({
    Name = "Auto Reject Selected Rarities",
    Default = false,
...
local rejectDropdown = mainSection1:AddDropdown({
    ...
    Callback = function(selected)
        ...
        if type(selected) == "table" then
            for _, v in pairs(selected) do Config.rejectNPCList[v] = true end
        end
    end,
    Save = true,
    Flag = "rejectRarities"
...
local autoWashToggle = mainSection2:AddToggle({
...
local autoPanToggle = mainSection2:AddToggle({
    Name = "Auto Pan",
    Default = false,
...
local autoTPToggle = mainSection3:AddToggle({
...
local mainSection4 = MainTab:AddSection({
    Name = "Other"
})
...
local autoClaimToggle = mainSection4:AddToggle({
...
local mainSection5 = MainTab:AddSection({
    Name = "Furniture & Menu"
})

local autoUnlockToggle = mainSection5:AddToggle({
...
-- PLAYER TAB
local playerSection1 = PlayerTab:AddSection({
    Name = "Movement"
})
...
local infJumpToggle = playerSection1:AddToggle({
    Name = "Infinite Jump",
    Default = false,
...
local noclipToggle = playerSection1:AddToggle({
    Name = "NoClip",
    Default = false,
    Callback = function(v)
        Config.noclip = v
...
local serveDelaySlider = settingsSection:AddSlider({
    Name = "Serve Delay",
    Min = 0.01,
    Max = 2,
    Default = 0.01,
...
local orderDelaySlider = settingsSection:AddSlider({
    Name = "Order Delay",
    Min = 0.1,
    Max = 2,
    Default = 0.01,
...
local washDelaySlider = settingsSection:AddSlider({
    Name = "Wash Delay",
    Min = 0.1,
    Max = 2,
    Default = 0.3,
...
local antiModToggle = modsSection:AddToggle({
    Name = "Instantly leaves when staff or higher ups join",
    Default = false,
    Callback = function(v)
        Config.antiMod = v
        if v then
...
configSection:AddLabel("Orion UI interface with saved element flags")

-- INGREDIENTS TAB
local ingredientsSection1 = IngredientsTab:AddSection({
...
local autoBuyIngredientsToggle = ingredientsSection1:AddToggle({
    Name = "Auto Buy",
    Default = false,
...
for _, ingredKey in ipairs(sortedIngredients) do
    local section = IngredientsTab:AddSection({
        Name = tostring(ingredKey)
    })
    local stockLabel = section:AddLabel("Stock: 0 / 0 (0%)")
    ingredientUIRefs[ingredKey] = stockLabel
    
    section:AddButton({
        Name = "Buy " .. tostring(ingredKey),
        Callback = function()
...
-- Auto Buy section
local shopSection1 = ShopTab:AddSection({
    Name = "Auto Buy"
})

-- Tables
local autoBuyTablesToggle = shopSection1:AddToggle({
    Name = "Auto Buy Table",
    Default = false,
...
local tableDropdown = shopSection1:AddDropdown({
    Name = "Select Tables",
    Default = "",
...
-- Chairs
local autoBuyChairsToggle = shopSection1:AddToggle({
    Name = "Auto Buy Chair",
    Default = false,
...
local chairDropdown = shopSection1:AddDropdown({
    Name = "Select Chairs",
    Default = "",
...
-- Materials
local autoBuyMaterialsToggle = shopSection1:AddToggle({
    Name = "Auto Buy Material",
    Default = false,
...
local materialDropdown = shopSection1:AddDropdown({
...
-- Paints
local autoBuyPaintsToggle = shopSection1:AddToggle({
    Name = "Auto Buy Paint",
    Default = false,
...
local paintDropdown = shopSection1:AddDropdown({
    Name = "Select Paints",
    Default = "",
...
-- Tiles
local autoBuyTilesToggle = shopSection1:AddToggle({
    Name = "Auto Buy Tile",
    Default = false,
...
local tileDropdown = shopSection1:AddDropdown({
    Name = "Select Tiles",
    Default = "",
...
-- Stoves
local autoBuyStovesToggle = shopSection1:AddToggle({
    Name = "Auto Buy Stove",
    Default = false,
...
local stoveDropdown = shopSection1:AddDropdown({
    Name = "Select Stoves",
    Default = "",
...
for _, group in ipairs(shopSections) do
    ...
    for _, entry in ipairs(sortedItems) do
        ...
        local itemSection = ShopTab:AddSection({
            Name = displayName
        })
        ...
        itemSection:AddLabel(priceDesc)

        local stockDesc = stock > 0 and ("Stock: " .. tostring(stock)) or "Stock: 0 | OUT OF STOCK"
        local stockLabel = itemSection:AddLabel(stockDesc)
        shopUIRefs[entry.Id] = stockLabel
